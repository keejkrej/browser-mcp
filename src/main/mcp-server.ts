...
import { getTabManager, getBrowserState, drainPickQueue, onResourceUpdate } from "./index.js";
  ...
  try {
    switch (name) {
      ...
      case "browser_snapshot": {
        ...
        `;
        return new Promise((resolve) => {
          tab.view.webContents.executeJavaScript(script).then((result: unknown) => {
            resolve({ content: [{ type: "text", text: (result as string) || "Empty snapshot" }] });
          }).catch((err: Error) => {
